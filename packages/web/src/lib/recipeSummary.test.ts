...
test('Full recipe (HP/LS): the Fragrance section is last; LS colorants sit in the after-dilution slot', () => {
  const ls = computedScent({
        fragrances: [{ name: 'Lemon', kind: 'essential-oil', percent: '1', vanillinPercent: '' }],
...
const SCENT_INPUT_HP = normalizeScentColor({
  fragrances: [{ name: 'Oak', percent: '3', vanillinPercent: '' }],
...
test('a blank scent row (no name, no dose) is not listed', () => {
  const blank = computedScent({ fragrances: [{ name: '', percent: '', vanillinPercent: '' }], colorants: [{ name: '', kind: 'mica', percent: '', portionKey: '' }], portions: [] }, { process: 'cp', totalOilGrams: 400, productGrams: 600 });
